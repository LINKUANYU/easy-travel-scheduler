...
async def image_proxy(url: str = Query(..., description="要代抓的第三方圖片網址")):
    ...
    headers = {
        ...
        "Accept": "image/avif,image/webp,image/apng,image/svg+xml,image/*,*/*;q=0.8", # 大聲宣告：我是來拿圖片的！
    ...
    async with AsyncSession(impersonate="chrome120") as client:
        try:
            ...
            print(f"⚠️ 第一次抓取失敗 ({url})，準備啟動 wsrv.nl 備援: {e}")
            # ==========================================
            # 第二層保險，開源代理wsrv.nl
            # ==========================================
            try:
                # wsrv.nl 是一個強大的圖片快取服務，幾乎能破解所有基礎防盜鏈
                wsrv_url = f"https://wsrv.nl/?url={url}"

                # 這次不帶 Referer，完全交給 wsrv.nl 去處理
                ...
                image_bytes = fallback_resp.content
                content_type = fallback_resp.headers.get("Content-Type", "image/jpeg")
                print(f"✅ wsrv.nl 備援成功救回圖片：{url}")

            except Exception as fallback_e:
                print(f"❌ 雙重抓取皆失敗 ({url}): {fallback_e}")
                # 如果連 wsrv.nl 都抓不到，代表圖片真的死了 (404)，安心拋出錯誤
