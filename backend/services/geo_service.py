...
def get_coordinates(location_name, base_location=""):
    ...
    # 請求主體
    data = {"textQuery": search_query, "languageCode": "zh-TW"}
    try:
...
def fetch_place_details_new(place_id: str) -> dict:
    ...
    try:
        ...
        raise HTTPException(
            status_code=502, detail=f"Places Details request failed: {e}"
        )

    if r.status_code != 200:
        # 常見：key 沒開 Places API (New) / 權限 / billing / referrer 限制
        raise HTTPException(
            status_code=502, detail=f"Places Details error: {r.status_code} {r.text}"
        )
