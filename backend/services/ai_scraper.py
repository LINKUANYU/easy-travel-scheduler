...
def master_scraper_workflow(location):
    ...
    if not url:
        ...
        raise ValueError(
            f"找不到「{location}」的相關旅遊資訊，請確認地點名稱是否正確或換個地點試試。"
        )
    ...
    try:
        ...
        try:
            ...
            if (
                "429" in fallback_msg
                or "quota" in fallback_msg
                or "exhausted" in fallback_msg
            ):
                raise ValueError("伺服器 AI 服務目前達到流量限制，請稍後再試。")
            if (
                "503" in fallback_msg
                or "overloaded" in fallback_msg
                or "unavailable" in fallback_msg
            ):
...
def run_web_scraping_workflow(location):
    ...
    for item in ai_gen_data:
        ...
        lat, lng, place_id, address, official_name = get_coordinates(
            original_name, base_location=location
        )
    ...
    if not valid_spots:
        raise ValueError(
            f"「{location}」的景點經過 Google 驗證後皆無效，可能為過於空泛的詞彙，請換個地點再試。"
        )
